...
- [ ] 1.1 — Project scaffolding (go module, directory structure, Makefile, `.go-arch-lint.yml`)
- [ ] 1.2 — Domain: Color value object and parsing (hex ↔ RGB ↔ HSL)
- [ ] 1.3 — Domain: Color operations (Blend, BlendBg, Lighten, Darken, Desaturate, ShiftHue)
- [ ] 1.4 — Domain: Palette entity (base24 struct, slot access, base16 fallbacks)
- [ ] 1.5 — Domain: Token value object and TokenSet aggregate
- [ ] 1.6 — Domain: ResolvedTheme aggregate
- [ ] 1.7 — Domain: Palette validation rules (luminance ordering, completeness)
- [ ] 1.8 — Domain: Error types (ParseError, ValidationError, GenerateError, SchemaVersionError)
- [ ] 1.9 — Domain: Schema version constants and file type registry
- [ ] 1.10 — Port interfaces (PaletteParser, PaletteSource, TokenDeriver, Mapper, Generator, ThemeStore)
- [ ] 1.11 — Port file structs (PaletteFile, UniversalFile, VimMappingFile, etc.)
- [ ] 1.12 — Port theme structs (VimTheme, GtkTheme, QssTheme, CssTheme, StylixTheme)
- [ ] 1.13 — Adapter: YAML palette parser (io.Reader → domain.Palette, common tinted-theming format only)
- [ ] 1.14 — Adapter: ThemeStore (filesystem — read/write theme dirs, symlink management)
- [ ] 1.15 — Adapter: Built-in palettes (//go:embed, PaletteSource impl, Get returns io.Reader)
  - [ ] 1.15a — tokyo-night-dark.yaml
  - [ ] 1.15b — gruvbox-dark.yaml
  - [ ] 1.15c — catppuccin-mocha.yaml
- [ ] 1.16 — Adapter: VersionedWriter (wraps io.Writer, prepends schema_version + kind header)
- [ ] 1.17 — Adapter: ValidatingReader (wraps io.Reader, peeks schema version, returns SchemaVersionError)
- [ ] 1.18 — Testdata: Tokyo Night Dark reference palette YAML
