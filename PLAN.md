...
**Goal:** Make flair fully usable as a Go library without requiring the flair CLI
to be installed. External Go programs should be able to import `pkg/flair` and:

1. Access built-in themes directly (no filesystem setup required)
2. Generate themes programmatically (palette → tokens)
3. Save themes to `~/.config/flair`
4. Select themes programmatically
5. Use the style viewer TUI

**Problem:** Currently, `pkg/flair` can only load themes from `~/.config/flair`
that were generated by the flair CLI. If the CLI isn't installed and no themes
exist, external programs get `no theme selected` errors.

**Solution:** Move essential logic from `internal/` to `pkg/` so the library is
self-contained. Built-in palettes, tokenization, and storage become public APIs.

#### Implementation Checklist

- [ ] 10.1 — Built-in Themes in `pkg/flair`
  - [ ] 10.1a — Move embedded palette YAML files to `pkg/flair/palettes/`
  - [ ] 10.1b — Create `pkg/flair/builtins.go` with `ListBuiltins()` and `LoadBuiltin(name)`
  - [ ] 10.1c — `LoadBuiltin()` returns a fully tokenized `*Theme` (no filesystem)
  - [ ] 10.1d — Built-in themes work without any `~/.config/flair` setup
  - [ ] 10.1e — Unit tests for built-in theme loading

- [ ] 10.2 — Public Palette and Tokenizer in `pkg/flair`
  - [ ] 10.2a — Create `pkg/flair/palette.go` — public Palette type (base24 colors)
  - [ ] 10.2b — Create `pkg/flair/color.go` — public Color type with hex parsing
  - [ ] 10.2c — Create `pkg/flair/tokenizer.go` — derive tokens from palette
  - [ ] 10.2d — Create `pkg/flair/parse.go` — parse palette YAML from `io.Reader`
  - [ ] 10.2e — Unit tests for palette parsing and tokenization

- [ ] 10.3 — Public Theme Store in `pkg/flair`
  - [ ] 10.3a — Create `pkg/flair/store.go` — read/write themes to `~/.config/flair`
  - [ ] 10.3b — `SaveTheme(theme *Theme)` — write theme files to config dir
  - [ ] 10.3c — `Select(name string)` — update symlinks to select a theme
  - [ ] 10.3d — `Install(name string)` — install a built-in theme to config dir
  - [ ] 10.3e — `InstallAll()` — install all built-in themes
  - [ ] 10.3f — Unit tests for store operations

- [ ] 10.4 — Convenience Functions
  - [ ] 10.4a — `flair.Default()` — load selected theme, fallback to built-in if none
  - [ ] 10.4b — `flair.MustLoad()` — panic variant for init-time loading
  - [ ] 10.4c — `flair.LoadOrDefault(name, fallback string)` — try named, fallback to built-in
  - [ ] 10.4d — `flair.EnsureInstalled()` — install built-ins if config dir empty
  - [ ] 10.4e — Unit tests for convenience functions

- [ ] 10.5 — Viewer Integration
  - [ ] 10.5a — Update viewer to work with built-in themes (no filesystem required)
  - [ ] 10.5b — `viewer.Run()` shows built-ins even if `~/.config/flair` doesn't exist
  - [ ] 10.5c — Option to install theme on selection (`OnSelect` can call `Install`)
  - [ ] 10.5d — Unit tests for viewer with built-ins

- [ ] 10.6 — Internal Refactoring
  - [ ] 10.6a — `internal/adapters/palettes/` imports from `pkg/flair/palettes/` (DRY)
  - [ ] 10.6b — `internal/adapters/tokenizer/` delegates to `pkg/flair/tokenizer.go`
  - [ ] 10.6c — CLI commands use `pkg/flair` where appropriate
  - [ ] 10.6d — Ensure `go-arch-lint check` still passes
  - [ ] 10.6e — No breaking changes to existing CLI behavior

- [ ] 10.7 — Documentation
  - [ ] 10.7a — README section: Using flair as a library (no CLI required)
  - [ ] 10.7b — Example: CLI with built-in flair theming (zero setup)
  - [ ] 10.7c — Example: programmatic theme generation
  - [ ] 10.7d — Example: embedding viewer with "install on select"
  - [ ] 10.7e — godoc comments for all new public APIs

#### Public API Surface

```go
// pkg/flair/builtins.go

// ListBuiltins returns names of all built-in themes.
func ListBuiltins() []string

// LoadBuiltin loads a built-in theme by name.
// Returns a fully tokenized Theme without any filesystem access.
func LoadBuiltin(name string) (*Theme, error)

// HasBuiltin returns true if the named built-in exists.
...
// Store manages themes in ~/.config/flair.
type Store struct {
    configDir string
}

// NewStore creates a Store with the default config directory.
func NewStore() *Store

// NewStoreAt creates a Store with a custom config directory.
...
// InstallAll installs all built-in themes.
func (s *Store) InstallAll() error

// Save writes a Theme to the config directory.
func (s *Store) Save(theme *Theme) error

// Select updates symlinks to point to the named theme.
func (s *Store) Select(name string) error

// Load reads the currently selected theme.
func (s *Store) Load() (*Theme, error)

// LoadNamed reads a specific theme by name.
func (s *Store) LoadNamed(name string) (*Theme, error)

// List returns all installed theme names.
...
```go
// pkg/flair/convenience.go

// Default loads the selected theme, falling back to "tokyo-night-dark" built-in.
func Default() *Theme

// MustLoad loads the selected theme or panics.
func MustLoad() *Theme

// LoadOrDefault tries to load a named theme, falls back to a built-in.
func LoadOrDefault(name, fallback string) *Theme

// EnsureInstalled installs all built-ins if the config dir is empty.
func EnsureInstalled() error
```

#### Usage Examples

**Zero-setup theming (uses built-in, no filesystem):**
...
func main() {
    // Works immediately — no CLI installation or setup required
    theme := flair.Default() // Falls back to built-in if nothing installed
    styles := flairlip.NewStyles(theme)

    fmt.Println(styles.Success.Render("Hello from flair!"))
}
```
...
```go
func main() {
    // Install all built-in themes if config dir is empty
    flair.EnsureInstalled()

    // Now Load() will find installed themes
    theme, _ := flair.Load()
    // ...
}
```
...
```go
func main() {
    // Parse a custom palette
    f, _ := os.Open("my-palette.yaml")
    palette, _ := flair.ParsePalette(f)

    // Generate theme (tokenize)
    theme := flair.NewTheme(palette)
...
func main() {
    ...
    viewer.Run(viewer.Options{
        OnSelect: func(name string) error {
            // Install built-in if not already installed
            if !store.Exists(name) && flair.HasBuiltin(name) {
...
- CAN be imported by `/internal` packages if needed (Phase 10: internal delegates to pkg)
- CAN depend on each other (`pkg/charm/lipgloss` → `pkg/flair`)

**After Phase 10:** `pkg/flair` becomes self-contained with:
- Built-in palettes (embedded YAML)
- Palette parsing and tokenization
- Theme storage and selection
- Works without flair CLI installation
...
8. **Phase 8** adds token overrides — users can customize tokens in palette.yaml.

9. **Phase 9** adds lualine and bufferline statusline integration.

10. **Phase 10** makes flair fully embeddable as a library. Moves built-in
    palettes, tokenization, and storage to `pkg/flair` so external Go programs
    can use flair themes without CLI installation. `internal/` delegates to
    `pkg/` to avoid duplication.
